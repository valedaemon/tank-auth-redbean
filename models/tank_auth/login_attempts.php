...
class Login_attempts extends CI_Model
{
	...
	{
		if (strlen($login) > 0) {
			$query = R::findOne($this->table_name, 'ip_address = :ip or login = :login', array('ip' => $ip_address, 'login' => $login));
		} else {
			$query = R::findOne($this->table_name, 'ip_address = :ip', array('ip' => $ip_address));
		}
		//TODO : Change this to return result count
	...
	{
		$save = R::dispense($this->table_name);
	...
	{
		$entries = R::find($this->table_name, 'ip_address = :ip and login = :login or time < :time', array('ip' => $ip_address, 'login' => $login, 'time' => time() - $expire_period));
